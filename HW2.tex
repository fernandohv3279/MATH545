...
There's actually only one boundary condition, thus, the solution isn't unique, $u(x)=-2bx+b$.
   ...
   \pagebreak
    ...
    \begin{enumerate}
        ...
        Now, let $v$ be a solution.

        The energy is given by:
        \begin{equation}\label{energy}
        E=\frac{1}{2}\int_{R} \left(\rho v_{t}^{2}(x,t)+Tv_{x}^{2}(x,t)\right)dx.
        \end{equation}
        We saw in class that this is a constant independent of time so by
        plugin $t=0$ in the expression above we get:
        \[E= \frac{1}{2}\int_{R}\left(\rho
        v_{t}^{2}(x,0)+Tv_{x}^{2}(x,0)\right)dx=\frac{1}{2}\int_{R}\left(\rho\psi^{2}+T\phi_{x}^{2}\right)dx=\frac{1}{2}\int_{R}(0+0)dx
    =0,\]
        which in conjunction with (\ref{energy}) implies:
        \[ \int_{R} \left(\rho v_{t}^{2}(x,t)+Tv_{x}^{2}(x,t)\right)dx=0.\]

        Since $\rho > 0$ and $T>0$, the vanishing theorem gives us:
        \[\rho v_{t}^{2}(x,t)+Tv_{x}^{2}(x,t)=0 \implies v_{t}=0, v_{x}=0.\]
        ...
        Therefore, $u(x,t)\equiv 0$ is the only solution of wave equation with $\phi \equiv 0$ and $\psi \equiv 0$.
