...
\subsection{Exercise 4}
		...
		\subsection{Exercise 8}
	For the hydrogen atom, if $ \int \left | u^2 \right | d \vec{x } =1$, at $t=0$, show that the same is true at all later times.
		
	\textbf{Solution:}

		If we want to show that $ \int \left | u^2 \right | d \vec{x } =1$ is true at all times, we can show that $\frac{d}{dt}\int \left | u^2 \right | d \vec{x } =0$.
	...
	\end{align*}
            ...
            In $\mathrm{R}^3$ space, let $B(r)$ be a ball with the origin as center and radius $r$.
			Then \[ \frac{d}{dt}\int \left | u^2 \right | d \vec{x }= \lim_{r \to \infty}\frac{d}{dt}\int_{B(r)} \left | u^2 \right | d \vec{x }= \frac{\mathrm{i}h}{2m}\lim_{r \to \infty}\iiint_{B(r)}^{} \nabla \cdot (\nabla u \cdot u^* -u\cdot \nabla u^*)d \vec{x }.\]
			From divergence theorem,
			\[\iiint_{B(r)} \nabla \cdot (\nabla u \cdot u^* -u\cdot \nabla u^*)d\vec{x}= \iint_{\partial B(r)}^{}(\nabla u \cdot u^* -u\cdot \nabla u^*) \cdot \bar{n} dS.	\]
...
From the initial condition $u(x,0)=x^2$, we know that $u_{xx}=2=u_t$.
		...
		$u(t,x)$ be the concentration of gas at position $x$ of the pipe at $t$.
...
We will choose (a) insulated solid to show the boundary condition in three-dimensional version.
