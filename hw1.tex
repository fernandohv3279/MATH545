...
\subsection{Exercise 1: Solve the first-order equation}
...
%Yingli changes the layout
			...
			\subsection{Exercise 2: Solve the equation $3u_y + u_{xy} =0.$}
...
%Yingli adds check part.	
	    ...
	    \subsection{Exercise 6: Solve the equation $\sqrt{1-x^2}u_x+u_y=0$ with the condition $u(0, y)=y.$}
		...
		\subsection{Exercise 8: Solve $au_x+bu_y+cu=0.$}
...
\section{Strauss section 1.3}
		\subsection{Exercise 4: Suppose that some particles which are
...
%Yingli seperates them with some explanation.
			...
			\subsection{Exercise 8: For the hydrigen atom, if $ \int \left | u^2 \right | d \vec{x } =1$, at $t=0$, show that the same is true at all later times.}
...
\section{Strauss section 1.4}
		\subsection{Exercise 1: By trial and error, find a solution of the diffusion equation $u_t=u_{xx}$ with the initial condition $u(x,0)=x^2.$}
...
\section{Strauss section 1.5}
		\subsection{ Exercise 1: Consider the problem
		%\begin{align*}
		%	\frac{d^2u}{dx^2}+u = 0 \\
		%	u(0)=0 \text{ and } u(L)=0,
		%\end{align*}
