...
\subsection{Exercise 4}
	...
	\]
	So in order to prove $\frac{d}{dt}\int|u|^2d\vec{x}=0$ it suffices to show that:
	\[
		\int\nabla \cdot (\nabla u \cdot u^* -u\cdot \nabla u^*)d\vec{x}=0.
	\]
...
be exchange of heat in the direction of $\vec{n}$, otherwise there would be heat
...
the real world that in this scenario we will get a unique function $u$ that
...
(both incompressible), in an enclosed domain after a very long time.
...
is constant and liquids are not compressible.
\end{document}
